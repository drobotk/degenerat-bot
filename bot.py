    ...
    for name in list(slash.commands.keys()):
        if name == "context":
            for _name in slash.commands["context"]:
                ...
                if not cmd.allowed_guild_ids or getattr(cmd, "global_command", False):
                    cmd.allowed_guild_ids = guild_ids
                    cmd.global_command = True
                else:
                    cmd.allowed_guild_ids = [
                        i for i in cmd.allowed_guild_ids if bot.get_guild(i) is not None
                    ]
                    if not cmd.allowed_guild_ids:
                        _log.warning(
                            f"Application context command { _name } has no allowed_guild_ids. Removing"
                        )
        ...
        if not cmd.allowed_guild_ids or getattr(cmd, "global_command", False):
            ...
            if not cmd.allowed_guild_ids:
                _log.warning(
                    f"Application command { name } has no allowed_guild_ids. Removing"
                )
