...
class Triggers(commands.Cog):
    def __init__(self, bot: DegeneratBot):
        ...
        self.text_triggers: dict[re.Pattern, str] = {}
        self.add_text_trigger(r"^sus$", "amogus")
        self.add_text_trigger(r"^amogus$", "ඞ")
        self.add_text_trigger(
            r"^erika$",
        ...
        self.add_text_trigger(r"^czy (?:juras|festo|seks|sex) ?\?+?$", "*Jeszcze nie.*")
        self.add_text_trigger(
            r"^tesco$",
        ...
        self.add_text_trigger(r"(?:^| )c?huj$", "ci w dupę")
        self.add_text_trigger(
        ...
        self.add_text_trigger(r"^\d*$", "Wiela???")
    ...
    def add_reaction_trigger(self, pattern: str, response: str):
        p = re.compile(pattern)
        self.reaction_triggers[p] = response

    def check_text_triggers(self, input: str) -> str:
        for pattern, res in self.text_triggers.items():
            if pattern.search(input):
                return res

        return self.spellcheck(input)

    def check_reaction_triggers(self, input: str) -> list[str]:
        return [
            reaction
            for pattern, reaction in self.reaction_triggers.items()
            if pattern.search(input)
        ]
    ...
    async def on_message(self, message: discord.Message):
        ...
        res = self.check_text_triggers(content)
        if res:
            await message.reply(res, mention_author=False)

        for reaction in self.check_reaction_triggers(content):
