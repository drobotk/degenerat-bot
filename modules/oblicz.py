...
class Oblicz(Cog):
    ...
    async def on_ready(self):
        self.illegal_in_output = [self.bot.http.token, *self.bot.http.token.split(".")]
        self.illegal_in_output = [
            *self.illegal_in_output,
    ...
    async def _oblicz(self, ctx: Union[SlashContext, MenuContext], expr: str):
        restrict = (
            ctx.author.id != self.bot.appinfo.owner.id
            if self.bot.appinfo is not None
            else True
        )
        ...
        if restrict and (logchannel := self.bot.get_channel(908412374673944657)):
            e = discord.Embed(
                title="Użył /oblicz",
                description=f"/oblicz `expr:{ expr }`",
        ...
        if restrict:
            for x in self.illegal_in_output:
                if x in wynik:
                    await ctx.send(
                        f"**Wykryto poufne treści w wyniku. Ten incident został zgłoszony do mojego administratora ({ self.bot.appinfo.owner.mention })**"
                    )
                    return
        ...
        try:
            if len(wynik) <= 2000:
                ...
                await ctx.send(
                    "**Błąd:** Rozmiar wyniku przekracza limit rozmiaru plików na tym serwerze."
                )

        except Exception as e:
            await ctx.send(f"**Nieoczekiwany błąd:** { str( e ) }")

    @cog_ext.cog_context_menu(name="Oblicz", target=ContextMenuType.MESSAGE)
