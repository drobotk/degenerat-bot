...
```""",
        ...
        self.end_triggers = {
            ("huj", "chuj"): "ci w dupę",
            ("można?", "można ?"): "*Można. Jak najbardziej. Jeszcze jak.*",
            (
                "dobrze?",
                "dobrze ?",
    ...
    def check_triggers(self, input: str) -> str:
        # end triggers first
        for t, content in self.end_triggers.items():
            if type(t) is not tuple:
                t = (t,)

            for tt in t:
                if input == tt or input.endswith(" " + tt):
                    return content

        for t, content in self.triggers.items():
            if type(t) is not tuple:
                t = (t,)
