...
class Cow( Cog ):
    ...
    @cog_ext.cog_slash(
        name = 'cow',
        description = 'Mądrości krowy'
    )
    async def _cow( self, ctx: SlashContext ):
        await ctx.defer()

        async with self.bot.http._HTTPClient__session.get('https://evilinsult.com/generate_insult.php?lang=en&type=json') as r:
            if not r.ok:
        ...
        async with self.bot.http._HTTPClient__session.get('https://translate.googleapis.com/translate_a/single', params = params ) as r:
            if not r.ok:
